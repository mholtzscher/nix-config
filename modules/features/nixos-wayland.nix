...
{ config, lib, ... }:
let
  cfg = config.myFeatures.nixosWayland;
in
{
  options.myFeatures.nixosWayland = {
    enable = lib.mkEnableOption "NixOS Wayland support" // {
      default = true;
      description = "XDG portals, Niri, DMS, and XWayland configuration";
    };
  };
