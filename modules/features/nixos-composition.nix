...
{ config, lib, ... }:
let
  cfg = config.myFeatures.nixosComposition;
in
{
  options.myFeatures.nixosComposition = {
    enable = lib.mkEnableOption "NixOS Niri composition" // {
      default = true;
      description = "Niri window manager + DMS composition settings";
    };
  };
