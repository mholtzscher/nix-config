...
{ config, lib, ... }:
let
  cfg = config.myFeatures.nixosWallpaper;
in
{
  options.myFeatures.nixosWallpaper = {
    enable = lib.mkEnableOption "NixOS Wallpaper daemon" // {
      default = true;
      description = "Awww wallpaper daemon with rotation timer";
    };
  };
