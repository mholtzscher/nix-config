...
usage() {
  cat <<'USAGE'
Usage: scripts/update-ghui.sh <version|latest> [--validate]

Examples:
  scripts/update-ghui.sh latest
  scripts/update-ghui.sh 0.6.0
  scripts/update-ghui.sh v0.6.0 --validate

Updates:
  pkgs/ghui/default.nix
USAGE
}
...
package_file="$repo_root/pkgs/ghui/default.nix"
...
if [[ "$requested_version" == "latest" ]]; then
  release_url="https://api.github.com/repos/kitlangton/ghui/releases/latest"
else
  version_without_v="${requested_version#v}"
  release_url="https://api.github.com/repos/kitlangton/ghui/releases/tags/v${version_without_v}"
fi
...
echo "Updated ghui: $old_version -> $new_version"
echo "Updated files:"
echo "  $package_file"
echo "Hashes:"
for hash_update in "${hash_updates[@]}"; do
